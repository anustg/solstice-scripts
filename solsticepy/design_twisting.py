...
class CRS:
	...
	def heliostatfield(self, field, hst_rho, slope, hst_w, hst_h, tower_h, tower_r=0.01, hst_z=0., num_hst=0., R1=0., fb=0., dsep=0., mac=None):
		...
		if field[-3:]=='csv':
			...
			self.field_azimuth=self.layout[:,8].astype(float)
	...
	def field_design_annual(self, dni_des,num_rays, nd, nh, weafile, method, Q_in_des=None, n_helios=None, zipfiles=False, gen_vtk=False, plot=False):
		...
		# creat a reference case for twisting heliostats
		efficiency_total, performance_hst=self.master.run(270, 74, num_rays, self.hst_rho, dni=1000, folder=os.path.join(self.casedir,'twist_ref'), gen_vtk=gen_vtk, printresult=False, verbose=self.verb, system=system)
		idx1=self.field_azimuth<0.1
		idx2=self.field_azimuth>359.9
		idx=idx1+idx2
		...
		for i in range(len(case_list)):    
			...
			if c not in run:
				...
				if elevation<1.:
					...
					efficiency_total, performance_hst=self.master.run(azimuth, elevation, num_rays, self.hst_rho, dni, folder=onesunfolder, gen_vtk=gen_vtk, printresult=False, verbose=self.verb, system=system, twisting=[closest_indices, self.helio_spil_ref])
		...
		sys.stderr.write("\n"+green('Design Point: \n'))		
		efficiency_total, performance_hst_des=self.master.run(azi_des, ele_des, num_rays, self.hst_rho, dni_des, folder=designfolder, gen_vtk=gen_vtk, printresult=False, verbose=self.verb, system=system, twisting=[closest_indices, self.helio_spil_ref])
	...
	def annual_oelt(self, num_rays, nd, nh, zipfiles=False, gen_vtk=False, plot=False):
		...
		closest_indices = np.array([np.argmin(np.abs(self.foc_ref - f)) for f in self.hst_foc])		
		oelt, ANNUAL=self.master.run_annual(nd=nd, nh=nh, latitude=self.latitude, num_rays=num_rays, num_hst=self.n_helios,rho_mirror=self.hst_rho, hst_aim_idx=self.hst_aim_idx, num_aperture=self.num_aperture, mac=self.mac, gen_vtk=gen_vtk, verbose=self.verb, system=system, twisting=[closest_indices, self.helio_spil_ref])
