...
class BD:
	...
	def surfacesareas(self, casefolder):
		'''
		Read the 'simul' file generated by Solstice to find the refelctive surfaces of the secondary reflector and the CPC
		'''
		rawfile = os.path.join(casefolder,'simul')
		rows = []
		index = 0
		...
		cpc_area=0.
		i=0
		while i<40: # It is assumed that the description of the reflectives surfaces will be within the first
			if 'secondary_reflector' in rows[i]:
				secref_area += float(rows[i][2])
			elif any("CPC_face" in string for string in rows[i]):
				cpc_area += float(rows[i][2])
			i=i+1
		self.A_secref = secref_area
		self.A_cpc = cpc_area
	...
	def field_design_annual(self,  dni_des, num_rays, nd, nh, weafile, method, Q_in_des=None, n_helios=None, zipfiles=False, gen_vtk=False, plot=False):
		...
		sys.stderr.write("\n"+green('Design Point: \n'))
		efficiency_total, performance_hst_des=self.master.run(azi_des, ele_des, num_rays, self.hst_rho, dni_des, folder=designfolder, gen_vtk=gen_vtk, printresult=False, system='beamdown', verbose=True)

		Qin=performance_hst_des[:,-1]
		Qsolar=performance_hst_des[0,0]

		# find surface areas of secondary reflector and CPC
		self.surfacesareas(designfolder)
		os.system('rm -rf %s'%designfolder)
